...
class Circle(Entity):
    ...
    def __init__(self, center: Coord, radius: float) -> None:
        self._center = center
        self._radius = radius
        self._polygon = center.to_shapely.buffer(radius, resolution=Circle.BUFFER_RESOLUTION)
        self._boundary = None
    ...
    def get_boundary_between(self, start: Coord, end: Coord) -> List[Coord]:
        ...
        while angle < great_angle:
            boundary1.append(self.center.shifted(self.radius + Circle.EPSILON, angle))
            angle += Circle.ANGLE_STEP
        boundary1.append(self.center.shifted(self.radius + Circle.EPSILON, great_angle))
        boundary1 = boundary1[::-1] if not boundary1[0].almost_equal(start, epsilon=0.1) else boundary1

        # clockwise boundary
        boundary2 = []
        angle = great_angle
        while angle > small_angle:
            boundary2.append(self.center.shifted(self.radius + Circle.EPSILON, angle))
            angle -= Circle.ANGLE_STEP
        boundary2.append(self.center.shifted(self.radius + Circle.EPSILON, small_angle))
        boundary2 = boundary2[::-1] if not boundary2[0].almost_equal(start, epsilon=0.1) else boundary2

        # choose shorter boundary
        return min([boundary1, boundary2], key=Path.compute_path_length)

    def compute_path_risk(self, path: List[Coord]) -> float:
        segments = [LineString([c1.to_shapely, c2.to_shapely]) for c1, c2 in zip(path[:-1], path[1:])]
        return sum([self.polygon.intersection(segment).length for segment in segments])

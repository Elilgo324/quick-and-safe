...
#     neighbor = []
#     for n in network:
#         neighbor.append(list(network[n].keys()))
#     return neighbor
...
def find_neighbor(network):
    ...
    :return: {node 1: [the neighbor nodes of node 1], ...}
    """
    neighbor = {n: [] for n in network}
    for n in network:
        neighbor[n] = list(network[n].keys())
    return neighbor
...
def main(network, source, destination, h, network_as_grpah):
    """
    the main function
    ...
    print('random_path_generator')
    for i in tqdm(range(pop)):
        # temp_path = random_path_generator(source, destination, neighbor)
        # temp_path = next(nx.all_simple_paths(network_as_grpah, source, destination))
        temp_path = nx.shortest_path(network_as_grpah, source, destination)
        population.append({
    ...
    print('Initialize the reference point')
    z = []  # reference point
    for i in range(nw):
        z.append(inf)
    for item in population:
    ...
    print('Add the Tchebycheff distance')
    for i in range(pop):
        item = population[i]
    ...
    # The main loop
    for iteration in tqdm(range(gen)):
        for i in range(pop):
